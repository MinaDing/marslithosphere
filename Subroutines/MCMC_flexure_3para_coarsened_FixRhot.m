function out = MCMC_flexure_3para_coarsened_FixRhot(gamma,Te_range,f_range,alpha_range,Te_isd,f_isd,alpha_isd,Te_ini,f_ini,alpha_ini,N,burnin,DEGS,FADM,FCOR,FADM_sd,FCOR_sd,degs,HH,Mij,Lwin)
...
prop_Var=[Te_isd f_isd alpha_isd]; % pre-assumed Gaussian variance

% Initialize Markov chain
X = zeros(dim,N); 
X(:,1) = [Te_ini;f_ini;alpha_ini]; % initial values
...
% Calculate prior 
currPrior = unifpdf(X(1,1),Te_range(1),Te_range(2))*unifpdf(X(2,1),f_range(1),f_range(2))*unifpdf(X(3,1),alpha_range(1),alpha_range(2)); % continuous uniform pdf
...
for k=2:N
    ...
    propPrior = unifpdf(Yprop(1),Te_range(1),Te_range(2))*unifpdf(Yprop(2),f_range(1),f_range(2))*unifpdf(Yprop(3),alpha_range(1),alpha_range(2)); % continuous uniform pdf
...
%    plot(DEGS,FCOR,'k');hold on;
    ...
    disp(['k=' num2str(k) ',accepted steps=' num2str(acc) ',Te=' num2str(X(1,k)) ' km, f=' num2str(X(2,k)) ' , alpha=' num2str(X(3,k))]);    
%     if mod(k,100)==0
%         save tmp.mat
%     end
...
fbest = mean(X(2,burnin:N));
fsd = std(X(2,burnin:N));
alphabest = mean(X(3,burnin:N));
alphasd = std(X(3,burnin:N));

%save tmp.mat
save(['Cap1_NP_3para_FixRhot_' num2str(gamma) '.mat']);
