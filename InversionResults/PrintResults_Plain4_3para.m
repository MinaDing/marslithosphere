...
FCOR_sdi = FCOR_sd;
capid = 4;
load Cap4_AP_3para_3751_25.mat
%load Cap4_AP_3para_2228_15.mat
degmin = 22;
degmax = 28;
...
% ezprint('portrait',[])
% print(['Cap' num2str(capid) '_3para.pdf'],'-dpdf')
%%
...
xlim([0 300]);
ylim([2200 3300]);
set(gca,'YMinorTick','on','XMinorTick','on','Xtick',0:50:300,'Ytick',2000:250:3000,'Layer','top')
ylabel '\rho_t (kg/m^3)'
...
rhotmin = quantile(X(2,burnin:N),0.16);
rhotmax = quantile(X(2,burnin:N),0.84);
[counts,centers]=hist(X(2,burnin:N),2200:bw:3500);
[f,xi,bw] = ksdensity(X(2,burnin:N),2200:bw:3500);
bar(centers,counts/sum(counts),'FaceColor',[0.7 0.7 0.7],'EdgeColor','none');
line(xi,f*bw,'color','r','linewidth',1);
xlim([2200 3300])
set(gca,'YMinorTick','on','XMinorTick','on','Xtick',2000:500:4000,'YAxisLocation','Right')
set(gca, 'Layer', 'top')
ylabel 'Posterior distribution (%)'
...
[counts,centers]=hist(X(3,burnin:N),0:bw:20);
[f,xi,bw] = ksdensity(X(3,burnin:N),0:bw:20);
barh(centers,counts/sum(counts),'FaceColor',[0.7 0.7 0.7],'EdgeColor','none');
line(f*bw,xi,'color','r','linewidth',1);
ylim([0 15])
set(gca,'YMinorTick','on','XMinorTick','on','Ytick',0:5:15,'Yticklabel',[])
set(gca, 'Layer', 'top')
xlabel 'Posterior distribution (%)'

ezprint('landscape',[])
print(['Cap' num2str(capid) '_3para_cov.pdf'],'-dpdf')
