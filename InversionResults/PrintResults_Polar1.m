...
line([burnin N],[Temax Temax],'color','k','linestyle','--');
line([burnin N],[Temin Temin],'color','k','linestyle','--');
line([burnin N],[Tebest Tebest],'color','k');
ylabel 'Te (km)'
...
ylim([1000 2000])
line([burnin burnin],get(gca,'YLim'),'color','k','linestyle','--')
line([burnin N],[rhotmax rhotmax],'color','k','linestyle','--');
line([burnin N],[rhotmin rhotmin],'color','k','linestyle','--');
line([burnin N],[rhotbest rhotbest],'color','k');
ylabel '\rhot (kg/m^3)'
...
ylim([0 1])
line([burnin burnin],get(gca,'YLim'),'color','k','linestyle','--')
line([burnin N],[fmax fmax],'color','k','linestyle','--');
line([burnin N],[fmin fmin],'color','k','linestyle','--');
line([burnin N],[fbest fbest],'color','k');
ylabel 'f'
...
xlim([0 300]);
ylim([1000 2000]);
set(gca,'YMinorTick','on','XMinorTick','on','Xtick',0:50:300,'Ytick',1000:500:2000,'Layer','top')
ylabel '\rho_t (kg/m^3)'
...
rhotmin = quantile(X(2,burnin:N),0.16);
rhotmax = quantile(X(2,burnin:N),0.84);
[counts,centers]=hist(X(2,burnin:N),1000:bw:2000);
[f,xi,bw] = ksdensity(X(2,burnin:N),1000:bw:2000);
bar(centers,counts/sum(counts),'FaceColor',[0.7 0.7 0.7],'EdgeColor','none');
line(xi,f*bw,'color','r','linewidth',1);
xlim([1000 2000])
set(gca,'YMinorTick','on','XMinorTick','on','Xtick',1000:500:2000,'YAxisLocation','Right')
set(gca, 'Layer', 'top')
ylabel 'Posterior distribution (%)'
...
set(gca,'Xtick',1000:500:2000,'XMinorTick','on','Ytick',0:0.5:1,'YMinorTick','on','Layer','top','Yticklabel',[])
ylim([0 1])
xlabel '\rho_t (kg/m^3)'
